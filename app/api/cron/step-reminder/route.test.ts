...
interface ClaimOptions { notificationType: string; occurrenceKey: string; userIds: readonly string[]; leaseOwner: string } interface FenceOptions { notificationType: string; occurrenceKey: string; userId: string; leaseOwner: string; claimToken: string } interface Ledger { state: 'pending' | 'claimed' | 'completed' | 'failed'; attempt: number; owner?: string; token?: string; previous?: FenceOptions }
...
async function claim(options: ClaimOptions): Promise<Array<{ user_id: string; claim_token: string }>> {
    ...
    return options.userIds.flatMap((userId) => { const entry = ledger.get(userId) ?? { state: 'pending' as const, attempt: 0 }; ledger.set(userId, entry); if (entry.state !== 'pending' || entry.attempt >= 5) return []; entry.state = 'claimed'; entry.attempt++; entry.owner = options.leaseOwner; entry.token = token(); return [{ user_id: userId, claim_token: entry.token }]; });
}
...
beforeEach(() => {
    ...
    mocks.release.mockImplementation(async (options: FenceOptions & { failureCode: string }) => { if (releaseFault === 'throw') throw new Error(PRIVATE); const entry = matches(options, releaseFault); if (!entry) return false; entry.previous = { ...options }; entry.state = entry.attempt >= 5 ? 'failed' : 'pending'; entry.owner = undefined; entry.token = undefined; return true; });
...
describe('GET /api/cron/step-reminder', () => {
    ...
    it('41ユーザーを20件以下の依存DB batchで処理する', async () => {
        ...
        expect(queries.every((query) => query.claimCalls > 0)).toBe(true);
    });
    it.each([
    ...
    it('invalid goalは新tokenで5回release後にclaim対象外となる', async () => { const ids = users(1); profileResolver = async () => ({ data: [{ id: ids[0], step_goal: 0, language: 'ja' }], error: null }); for (let attempt = 0; attempt < 5; attempt++) expect((await GET(request())).status).toBe(503); const final = await GET(request()); expect(final.status).toBe(200); expect(await final.json()).toMatchObject({ claimed: 0, skipped: 1, sent: 0, failedUsers: 0 }); expect(mocks.release).toHaveBeenCalledTimes(5); const tokens = mocks.release.mock.calls.map(([options]) => options.claimToken); expect(new Set(tokens).size).toBe(5); expect(tokens[0]).not.toBe(tokens[1]); expect(mocks.release.mock.calls.every(([options]) => options.userId === ids[0] && options.failureCode === 'SOURCE_DATA_UNAVAILABLE')).toBe(true); expect(ledger.get(ids[0])).toMatchObject({ state: 'failed', attempt: 5 }); expect(queries).toHaveLength(10); expect(mocks.sendPush).not.toHaveBeenCalled(); });
    it.each([
        ['error', { data: [], error: { message: PRIVATE } }], ['null', { data: null, error: null }],
    ...
    it('共有invalid/raw capとprofile/steps不正を隔離し、正常ユーザーを送信する', async () => {
        ...
        expect(await response.json()).toMatchObject(
            { checked: 5, eligible: 1, claimed: 3, completedDelivery: 1,
                released: 2, sent: 1, failed: 0, failedUsers: 4, outboxFailures: 0 });
        expect(categories()).toEqual(expect.arrayContaining(['subscriptions-validation',
            'subscriptions-user-limit', 'profiles-validation', 'steps-validation']));
    });
    it.each([['profiles', 'foreign+valid'], ['steps', 'foreign+missing']])(
        '%sに%s行がある場合、該当batchを送信せず次batchを継続する', async (source) => {
            ...
            expect(body).toMatchObject({ checked: 21, claimed: 21, eligible: 1,
                completedDelivery: 1, released: 20, sent: 1, failed: 0, failedUsers: 20 });
            expect(mocks.sendPush).toHaveBeenCalledTimes(1); expect(mocks.sendPush.mock.calls[0][0]).toBe(ids[20]);
            expect(categories()).toContain(`${source}-foreign-row`);
        });
    it('未記録と記録済み0を有効な0歩とし、70%境界とja/enを区別する', async () => {
        ...
        expect(mocks.sendPush.mock.calls[0][2].body).toContain('今日の歩数: 0 / 10,000');
        ...
        expect(await response.json()).toMatchObject(
            { checked: 4, claimed: 4, eligible: 3, suppressed: 1, completedDelivery: 3 });
    });
