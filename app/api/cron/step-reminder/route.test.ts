...
interface ClaimOptions { notificationType: string; occurrenceKey: string; userIds: readonly string[]; leaseOwner: string } interface FenceOptions { notificationType: string; occurrenceKey: string; userId: string; leaseOwner: string; claimToken: string } interface Ledger { state: 'pending' | 'claimed' | 'completed'; attempt: number; owner?: string; token?: string; previous?: FenceOptions }
...
describe('GET /api/cron/step-reminder', () => {
    ...
    it('未記録と記録済み0を有効な0歩とし、70%境界とja/enを区別する', async () => {
        ...
        expect(await response.json()).toMatchObject(
            { checked: 4, claimed: 4, underGoal: 3, suppressed: 1, completedDelivery: 3 });
    });
