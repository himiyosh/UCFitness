...
interface ChallengeListRow {
    ...
    group: { id: string; is_public: boolean } | null;
    created_at: string;
...
export async function GET(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const [groupAccess, participationsResult, membershipsResult] = await Promise.all([
            groupId
                ? authorizeGroupChallenge(groupId, userId, 'view', 'challenge:list')
                : Promise.resolve({ allowed: true as const, role: null }),
            supabaseAdmin
        ...
        const listDenial = getGroupChallengeDenial(
            groupAccess,
            'Failed to authorize group challenge access',
            'Group not found',
        );
        if (listDenial) {
            return NextResponse.json({ error: listDenial.error }, { status: listDenial.status });
        }
        if (participationsResult.error) {
            reportError('challenge:list:participations', participationsResult.error, { userId });
            return NextResponse.json({ error: 'Failed to fetch challenges' }, { status: 500 });
        }
        if (
            (participationsResult.count !== null && participationsResult.count > 1000)
            || (membershipsResult.count !== null && membershipsResult.count > 1000)
        ) {
            reportError('challenge:list:access-scope', new Error('Challenge access scope exceeded 1000 rows'), {
                userId,
            });
            return NextResponse.json({ error: 'Failed to fetch challenges' }, { status: 500 });
        }
        if (membershipsResult.error) {
            reportError('challenge:list:memberships', membershipsResult.error, { userId });
            return NextResponse.json({ error: 'Failed to fetch challenges' }, { status: 500 });
        }
        ...
        const memberGroupIds = new Set(
            membershipsResult.data?.map((row) => row.group_id) ?? [],
        );
        ...
        const visibleIds = (data ?? [])
            .filter((challenge) => challenge.type === 'INDIVIDUAL'
                || (
                    challenge.group_id !== null
...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        ...
        if (type === 'GROUP') {
            ...
            const access = await authorizeGroupChallenge(
                group_id,
                session.user.id,
                'manage',
                'challenge:create',
            );
            const createDenial = getGroupChallengeDenial(
                access,
                'Failed to authorize group challenge creation',
                'Group not found',
            );
            if (createDenial) {
                return NextResponse.json({ error: createDenial.error }, { status: createDenial.status });
            }
        }
