...
function expectFixedFailure(stage: string): void {
    const [operation, error] = mocks.reportError.mock.calls.at(-1) ?? [];
    expect(operation).toBe('external/ranking'); expect(error).toBeInstanceOf(AppError);
    expect(error).toMatchObject({
        message: 'External ranking request failed', code: 'EXTERNAL_RANKING_UNAVAILABLE',
        context: { stage }, cause: undefined,
    });
}
beforeEach(() => {
    vi.clearAllMocks();
...
describe('GET /api/external/ranking', () => {
    ...
    it('明示groupIdも同じレスポンス形と4クエリを維持する', async () => {
        ...
        expectFixedFailure('groups'); results.groups.count = 1; results.group_members.count = 1_001;
        expect((await GET(request(G1))).status).toBe(500); expectFixedFailure('members');
    });
