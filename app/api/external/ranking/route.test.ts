...
function createQuery(table: Table): Query {
    const chain = {
        ...
        then: <T1 = QueryResult, T2 = never>(
            ok?: ((value: QueryResult) => T1 | PromiseLike<T1>) | null,
            fail?: ((reason: unknown) => T2 | PromiseLike<T2>) | null,
        ): Promise<T1 | T2> => Promise.resolve(results[table]).then(ok, fail),
    } as Query;
...
beforeEach(() => {
    ...
    queryCalls = [];
    results = {
        groups: { data: [{ id: G1, name: 'Group One' }, { id: G2, name: 'Group Two' }], error: null },
        group_members: { data: [
            { group_id: G1, user_id: U2 }, { group_id: G1, user_id: U1 },
            { group_id: G1, user_id: U3 }, { group_id: G1, user_id: U4 }, { group_id: G1, user_id: U5 },
            { group_id: G2, user_id: U3 },
        ], error: null },
        users: { data: [U1, U2, U3, U4, U5].map((id, index) => (
            { id, name: `User ${index + 1}`, username: `user-${index + 1}`, image: null }
        )), error: null },
        daily_steps: { data: [
            { user_id: U1, steps: 300 }, { user_id: U2, steps: 300 }, { user_id: U3, steps: 100 },
            { user_id: U4, steps: 0 },
        ], error: null },
    };
...
describe('GET /api/external/ranking', () => {
    it.each([
        ['CRON_SECRET未設定', undefined, SECRET], ['Authorization不一致', SECRET, 'wrong-secret'],
        ['Authorization欠落', SECRET, null],
    ...
    it('明示groupの不存在を404、DB障害を固定500として区別する', async () => {
        ...
        results.groups = { data: null, error: { message: RAW } };
        ...
        results.groups = { data: { id: G2, name: 'Foreign' }, error: null };
        expect((await GET(request(G1))).status).toBe(500);
    });
    it('全グループを4クエリで取得し、正歩数だけに安定した連続順位を付ける', async () => {
        ...
        expect(queryCalls).toEqual([
            ...
            ['group_members', 'select', ['group_id, user_id']], ['group_members', 'in', ['group_id', [G1, G2]]],
            ['users', 'select', ['id, name, username, image']], ['users', 'in', ['id', [U2, U1, U3, U4, U5]]],
            ['daily_steps', 'select', ['user_id, steps']], ['daily_steps', 'eq', ['date', '2026-07-25']],
            ['daily_steps', 'in', ['user_id', [U2, U1, U3, U4, U5]]],
        ]);
    });
    it('明示groupIdも同じレスポンス形と4クエリを維持する', async () => {
        results.groups.data = { id: G1, name: 'Group One' };
        ...
        expect(queryCalls).toEqual([
            ['groups', 'select', ['id, name']], ['groups', 'eq', ['id', G1]], ['groups', 'maybeSingle', []],
            ['group_members', 'select', ['group_id, user_id']], ['group_members', 'in', ['group_id', [G1]]],
            ['users', 'select', ['id, name, username, image']], ['users', 'in', ['id', [U2, U1, U3, U4, U5]]],
            ['daily_steps', 'select', ['user_id, steps']], ['daily_steps', 'eq', ['date', '2026-07-25']],
            ['daily_steps', 'in', ['user_id', [U2, U1, U3, U4, U5]]],
        ]);
    ...
    it('明示groupと複数group中の空memberを正常な空集合として扱う', async () => {
        results.groups.data = { id: G1, name: 'Group One' }; results.group_members.data = [];
        const explicit = await GET(request(G1));
        expect([explicit.status, await explicit.json()]).toEqual([200, { date: '2026-07-25', groups: [] }]);
        results.groups.data = [{ id: G1, name: 'Group One' }, { id: G2, name: 'Group Two' }];
        results.group_members.data = [{ group_id: G1, user_id: U1 }];
        results.users.data = [{ id: U1, name: 'User 1', username: 'user-1', image: null }];
        results.daily_steps.data = [{ user_id: U1, steps: 300 }];
    ...
    ] as const)('%sのerrorとnull dataを固定500へ分離する', async (table, stage) => {
        for (const result of [{ data: [], error: { message: RAW } }, { data: null, error: null }]) {
    ...
    it.each([
        ...
        results[table].data = typeof change === 'function' ? change(results[table].data) : change;
        expect((await GET(request())).status).toBe(500);
    ...
    it('unexpected errorのraw値をログとレスポンスへ渡さない', async () => {
        ...
        expect((mocks.reportError.mock.calls[0][1] as Error).message)
            .toBe('External ranking request failed');
