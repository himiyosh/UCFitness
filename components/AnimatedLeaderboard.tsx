    ...
    return (
        <div className="space-y-6">
            ...
            <div className="overflow-hidden rounded-xl bg-white shadow-sm border border-[var(--theme-primary)]/10 transition-all duration-300">
                {/* Panel Header: Scope Tabs + Period Badge */}
                <div className="px-4 py-3 border-b border-gray-100">
                    <div className="flex items-center justify-between gap-3">
                        {/* Scope Tabs */}
                        <div className="flex overflow-x-auto scrollbar-hide gap-1.5 min-w-0">
                            <button
                                onClick={() => { setScope('GLOBAL'); setPage(1); }}
                                className={`cursor-pointer whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-bold transition-all ${scope === 'GLOBAL' ? 'bg-[var(--theme-primary)] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
                            >
                                🌍 {t('titleGlobal')}
                            </button>
                            {allGroupRankings.map((group, idx) => (
                                <button
                                    key={group.keyword}
                                    onClick={() => { setScope(idx); setPage(1); }}
                                    className={`cursor-pointer whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 ${scope === idx ? 'bg-[var(--theme-primary)] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
                                >
                                    {group.image_url && <img src={group.image_url} className="w-3.5 h-3.5 rounded-full" alt="" />}
                                    {group.keyword}
                                </button>
                            ))}
                        </div>
                ...
                {scope === 'GLOBAL' ? (
                    ...
                    <div className="bg-white px-0 relative overflow-hidden flex flex-col min-h-[460px]">
                        <FadeInWrapper key={`global-${period}`}>
                    ...
                    /* ===== GROUP CONTENT ===== */
                    (() => {
                        const groupData = allGroupRankings[scope as number];
                        ...
                        return (
                            <div>
                                {/* Summary Cards */}
                                <div className="grid grid-cols-2 gap-3 sm:gap-4 p-4">
                                    {/* My Rank Display */}
                                    {myRankEntry && (
                                            ...
                                            <div className="relative z-10 p-3 sm:p-4 text-white flex items-center justify-between">
                                                <div className="min-w-0 flex-1">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        {groupData.image_url && (
                                                            <img src={groupData.image_url} className="w-4 h-4 rounded-full border border-white/30" alt="" />
                                                        )}
                                    ...
                                    <div className={`relative rounded-xl overflow-hidden shadow-sm border border-gray-100 bg-white p-3 sm:p-4 flex flex-col justify-center animate-in fade-in zoom-in duration-300 delay-75 ${!myRankEntry ? 'col-span-2' : ''}`}>
            ...
            {/* Group Competition Ranking - below the main panel */}
            {scope === 'GLOBAL' && groupCompetitionRankings && groupCompetitionRankings[period] && (
