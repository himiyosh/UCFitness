...
export default function AnimatedLeaderboard({ userEmail, userId, allGlobalRankings, allGroupRankings, groupCompetitionRankings }: AnimatedLeaderboardProps) {
    ...
    // Memoize global list processing & pagination
    ...
    }, [currentGlobal.length, page]);

    // Ensure currentPage is valid if data changes - Moved to top level to fix Hook Rule violation
    useEffect(() => {
    ...
    const { paginatedItems, maxSteps } = useMemo(() => {
        const startIndex = (SafePage - 1) * ITEMS_PER_PAGE;
        const items = currentGlobal.slice(startIndex, startIndex + ITEMS_PER_PAGE).map((entry, idx) => ({
            ...entry,
            originalRank: startIndex + idx + 1
        }));
        const max = Math.max(...items.map(g => g.steps)) || 1;
        return { paginatedItems: items, maxSteps: max };
    }, [currentGlobal, SafePage]);

    // Memoize TopUsersChart data
    const currentGlobalWithRank = useMemo(() =>
    ...
    const groupStats = useMemo(() => {
        if (allGroupRankings.length === 0) return null;
        if (selectedGroupIndex >= allGroupRankings.length) return null;
    ...
    }, [allGroupRankings, selectedGroupIndex, period, userEmail, userId, groupCompetitionRankings]);
                            ...
                            {groupStats ? (
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                                        {/* My Rank Display */}
                                        {groupStats.myRankEntry && (
                                            <Link href={`/group/${groupStats.groupData.groupId}`} className="relative rounded-xl overflow-hidden shadow-lg animate-in fade-in zoom-in duration-300 group block hover:ring-2 hover:ring-offset-2 hover:ring-indigo-500 transition-all">
                                                {/* Background Image or Gradient */}
                                                <div className="absolute inset-0 z-0">
                                                    {groupStats.groupData.header_image_url ? (
                                                        <>
                                                            <img
                                                                src={groupStats.groupData.header_image_url}
                                                                alt=""
                                                ...
                                                <div className="relative z-10 p-3 sm:p-4 text-white flex items-center justify-between">
                                                    <div className="min-w-0 flex-1">
                                                        <div className="flex items-center gap-2 mb-1">
                                                            {groupStats.groupData.image_url && (
                                                                <img src={groupStats.groupData.image_url} className="w-4 h-4 rounded-full border border-white/30" />
                                                            )}
                                                            <p className="text-white/80 text-[10px] sm:text-xs font-bold uppercase tracking-wider truncate">{t('yourRank')}</p>
                                                        </div>
                                                        <div className="flex items-baseline gap-1 sm:gap-2 flex-wrap">
                                                            <span className="text-2xl sm:text-3xl font-black leading-none">#{groupStats.myRank}</span>
                                                            <span className="text-[10px] sm:text-sm font-medium opacity-90 line-clamp-1">{t('inGroup', { group: groupStats.groupData.keyword })}</span>
                                                        </div>
                                                    </div>
                                                    <div className="text-right ml-2 shrink-0">
                                                        <div className="text-lg sm:text-2xl font-bold tracking-tight leading-none mb-1">{groupStats.myRankEntry.steps.toLocaleString()}</div>
                                        ...
                                        <div className="relative rounded-xl overflow-hidden shadow-sm border border-gray-100 bg-white p-3 sm:p-4 flex flex-col justify-center animate-in fade-in zoom-in duration-300 delay-75">
                                            ...
                                            <div className="flex items-baseline gap-2 flex-wrap">
                                                {groupStats.groupRank ? (
                                                    <>
                                                        <span className="text-2xl sm:text-3xl font-black text-indigo-600 tracking-tight leading-none">#{groupStats.groupRank}</span>
                                                        <span className="text-xs sm:text-sm font-bold text-gray-400">/ {groupStats.totalGroups}</span>
                                                    </>
                                                ) : (
                                            ...
                                            <div className="mt-2 flex items-center gap-2 pt-2 border-t border-gray-50">
                                                <div className="flex flex-col min-w-0">
                                                    ...
                                                    <span className="text-xs sm:text-sm font-bold text-gray-700 truncate">{groupStats.averageSteps.toLocaleString()} <span className="text-[10px] font-normal text-gray-400">{t('steps')}</span></span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <FadeInWrapper key={`${groupStats.groupData.keyword}-${period}`}>
                                        <GroupRankingPanel
                                            keyword={groupStats.groupData.keyword}
                                            groupId={groupStats.groupData.groupId}
                                            neighbors={groupStats.displayRankings}
                                            userEmail={userEmail}
                                            index={0}
                                            totalCount={1}
                                        />
