...
export default function AnimatedLeaderboard({ userId, allGlobalRankings, allGroupRankings, groupCompetitionRankings }: AnimatedLeaderboardProps) {
    ...
    // scope: 'GLOBAL' or group index (number)
    const [scope, setScope] = useState<'GLOBAL' | number>(allGroupRankings.length > 0 ? 0 : 'GLOBAL');
    ...
    // Handle Scope Switch
    const handleScopeSwitch = (newScope: 'GLOBAL' | number) => {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Scope Tabs: Global | Group1 | Group2 ... */}
            <div className="flex overflow-x-auto pb-1 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0 gap-2">
                {/* Global Tab */}
                <button
                    onClick={() => handleScopeSwitch('GLOBAL')}
                    className={`cursor-pointer whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 ${!isMidnight ? (scope === 'GLOBAL' ? 'bg-[var(--theme-primary)] text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50') : ''}`}
                    style={isMidnight ? (scope === 'GLOBAL' ? {
                ...
                {allGroupRankings.map((group, idx) => (
                    <button
                        key={group.keyword}
                        onClick={() => handleScopeSwitch(idx)}
                        className={`cursor-pointer whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 ${!isMidnight ? (scope === idx ? 'bg-[var(--theme-primary)] text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50') : ''}`}
                        style={isMidnight ? (scope === idx ? {
                            background: 'linear-gradient(135deg, #6366f1 0%, #7c3aed 100%)',
                            color: '#ffffff',
                            boxShadow: '0 4px 20px -3px rgba(99,102,241,0.5), inset 0 1px 0 rgba(255,255,255,0.15)',
                            border: '1px solid rgba(165,180,252,0.3)',
                    ...
                    >
                        {group.image_url && <img src={group.image_url} className="w-4 h-4 rounded-full border border-white/30" alt="" />}
                        {group.keyword}
                    </button>
                ))}
            </div>
