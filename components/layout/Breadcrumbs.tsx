...
export default async function Breadcrumbs({ items = [], className = "" }: BreadcrumbsProps) {
    ...
    return (
        <nav className={`w-full min-w-0 overflow-x-auto overscroll-x-contain text-sm font-medium text-[var(--color-text-muted)] ${className}`} aria-label={t('breadcrumbLabel')}>
            <ol className="flex w-max min-w-full items-center gap-1 sm:gap-2">
                ...
                {items.map((item, index) => (
                    <li key={item.href ?? `breadcrumb-${index}`} className="flex items-center gap-1 sm:gap-2">
                        ...
                        {item.href ? (
                            <Link href={item.href} className="inline-flex min-h-[44px] items-center whitespace-nowrap rounded-lg px-2 transition-colors hover:text-[var(--color-primary-strong)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]">
                                {item.label}
                            </Link>
                        ) : (
                            <span className="whitespace-nowrap font-semibold text-[var(--color-text)]" aria-current="page">
                                {item.label}
                            </span>
                        )}
                    </li>
