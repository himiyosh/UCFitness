...
describe('WalkingRoutes duration validation', () => {
    it.each([
        ['1e2', undefined], ['1.5', undefined], ['3abc', undefined],
    ...
    it('native badInputを送信せず、エラーDOM反映後に毎回入力へfocusする', async () => {
        ...
        try {
            ...
            await duration.pressSequentially('1e309');
            expect(await duration.evaluate((input) => ({
                badInput: (input as HTMLInputElement).validity.badInput, value: (input as HTMLInputElement).value,
            }))).toEqual({ badInput: true, value: '' });
