...
export function getPushEndpointOwnershipKey(endpoint: unknown): string | null {
    ...
    try {
        ...
        if (url.protocol !== 'https:' || url.username || url.password || !allowedHost) {
            return null;
        }

        url.hash = '';
        const ownershipKey = url.href.replace(
            PERCENT_ENCODED_BYTE,
            (_encoded, hex: string) => {
                const character = String.fromCharCode(Number.parseInt(hex, 16));
                return ASCII_UNRESERVED.test(character)
                    ? character
                    : `%${hex.toUpperCase()}`;
            },
        );
