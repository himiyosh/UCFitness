...
describe('sendWebPushNotification', () => {
    ...
    it('ローカライズ済みpayloadを送る場合、RFC 8291形式で復号できる', async () => {
        ...
        const invalid = { id: '10000000-0000-4000-8000-000000000001', user_id: '00000000-0000-4000-8000-000000000001', endpoint: 'https://fcm.googleapis.com/invalid', p256dh: toBase64Url(receiverPublicKey), auth: 'a', user_agent: 'Browser', created_at: '2026-07-01T00:00:00Z' }; const valid = { ...invalid, id: '10000000-0000-4000-8000-000000000002', user_id: '00000000-0000-4000-8000-000000000002', endpoint: 'https://fcm.googleapis.com/valid', auth: toBase64Url(authSecret) }; const unrelated = { ...valid, id: '10000000-0000-4000-8000-000000000003', user_id: '00000000-0000-4000-8000-000000000003', endpoint: 'https://fcm.googleapis.com/unrelated' }; const invalidRows = Array.from({ length: 21 }, (_, index) => ({ ...invalid, id: `10000000-0000-4000-8001-${String(index + 1).padStart(12, '0')}` })); const prepared = await preparePushSubscriptionSnapshot([...invalidRows, valid]); expect(prepared.cappedUserIds).toEqual([invalid.user_id]); expect(prepared.byUser.get(valid.user_id)).toHaveLength(1); const sameUser = await preparePushSubscriptionSnapshot([valid, { ...valid, id: '10000000-0000-4000-8000-000000000004' }]); expect(sameUser.invalidUserIds).toEqual([valid.user_id]); expect(sameUser.byUser.size).toBe(0); const crossUser = await preparePushSubscriptionSnapshot([valid, { ...unrelated, id: '10000000-0000-4000-8000-000000000005', endpoint: valid.endpoint }]); expect(crossUser.invalidUserIds).toEqual([valid.user_id, unrelated.user_id]); expect(crossUser.byUser.size).toBe(0); const invalidDuplicate = await preparePushSubscriptionSnapshot([{ ...invalid, endpoint: valid.endpoint }, valid, unrelated]); expect(invalidDuplicate.invalidUserIds).toEqual(expect.arrayContaining([invalid.user_id, valid.user_id])); expect(invalidDuplicate.byUser.get(unrelated.user_id)).toHaveLength(1); const cappedDuplicate = await preparePushSubscriptionSnapshot([...Array.from({ length: 21 }, (_, index) => ({ ...valid, id: `10000000-0000-4000-8001-${String(index + 1).padStart(12, '0')}`, user_id: invalid.user_id, endpoint: index === 20 ? unrelated.endpoint : `https://fcm.googleapis.com/capped-${index}` })), unrelated]); expect(cappedDuplicate.cappedUserIds).toEqual([invalid.user_id]); expect(cappedDuplicate.invalidUserIds).toEqual(expect.arrayContaining([invalid.user_id, unrelated.user_id])); expect(cappedDuplicate.byUser.has(unrelated.user_id)).toBe(false); await expect(preparePushSubscriptionSnapshot([invalid, { ...valid, id: invalid.id }])).rejects.toMatchObject({ reason: 'data' });
