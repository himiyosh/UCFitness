...
describe('preparePushSubscriptionSnapshot endpoint normalization', () => { it('資格情報を隔離し、hrefのfragment除去だけを所有権正規化に使う', async () => { const p256dh = 'BGsX0fLhLEJH-Lzm5WOkQPJ3A32BLeszoPShOUXYmMKWT-NC4v4af5uO5-tKfA-eFivOM1drMV7Oy7ZAaDe_UfU'; const auth = 'A'.repeat(22); const user = (index: number) => `00000000-0000-4000-8000-${String(index).padStart(12, '0')}`; const row = (index: number, userIndex: number, endpoint: string) => ({ id: `30000000-0000-4000-8000-${String(index).padStart(12, '0')}`, user_id: user(userIndex), endpoint, p256dh, auth, user_agent: 'Browser', created_at: '2026-07-01T00:00:00Z' }); const credentialed = await preparePushSubscriptionSnapshot([row(1, 1, 'https://account:secret@fcm.googleapis.com/fcm/send/credentialed'), row(2, 2, 'https://fcm.googleapis.com/fcm/send/unrelated')]); expect(credentialed.invalidUserIds).toEqual([user(1)]); expect([...credentialed.byUser.keys()]).toEqual([user(2)]); const canonical = await preparePushSubscriptionSnapshot([row(3, 1, 'https://fcm.googleapis.com/fcm/send/canonical'), row(4, 2, 'https://FCM.GOOGLEAPIS.COM:443/fcm/send/canonical#ignored'), row(5, 3, 'https://fcm.googleapis.com/fcm/send/unrelated')]); expect(new Set(canonical.invalidUserIds)).toEqual(new Set([user(1), user(2)])); expect([...canonical.byUser.keys()]).toEqual([user(3)]); const emptyQuery = await preparePushSubscriptionSnapshot([row(6, 1, 'https://fcm.googleapis.com/fcm/send/query'), row(7, 2, 'https://fcm.googleapis.com/fcm/send/query?')]); expect(emptyQuery.invalidUserIds).toEqual([]); expect([...emptyQuery.byUser.keys()]).toEqual([user(1), user(2)]); const encodedPath = await preparePushSubscriptionSnapshot([row(8, 1, 'https://fcm.googleapis.com/fcm/send/encoded%2Fpath'), row(9, 2, 'https://fcm.googleapis.com/fcm/send/encoded/path')]); expect(encodedPath.invalidUserIds).toEqual([]); expect([...encodedPath.byUser.keys()]).toEqual([user(1), user(2)]); }); });
describe('preparePushSubscriptionSnapshot RFC3986 aliases', () => { it('unreserved aliasesとreserved encodingを区別して全ownerを隔離する', async () => { const p256dh = 'BGsX0fLhLEJH-Lzm5WOkQPJ3A32BLeszoPShOUXYmMKWT-NC4v4af5uO5-tKfA-eFivOM1drMV7Oy7ZAaDe_UfU'; const auth = 'A'.repeat(22); const user = (index: number) => `00000000-0000-4000-8000-${String(index).padStart(12, '0')}`; const row = (index: number, userIndex: number, endpoint: string) => ({ id: `40000000-0000-4000-8000-${String(index).padStart(12, '0')}`, user_id: user(userIndex), endpoint, p256dh, auth, user_agent: 'Browser', created_at: '2026-07-01T00:00:00Z' }); const assertAlias = async (first: string, second: string, offset: number) => { const prepared = await preparePushSubscriptionSnapshot([row(offset, 1, first), row(offset + 1, 2, second), row(offset + 2, 3, 'https://fcm.googleapis.com/fcm/send/unrelated')]); expect(new Set(prepared.invalidUserIds)).toEqual(new Set([user(1), user(2)])); expect([...prepared.byUser.keys()]).toEqual([user(3)]); }; await assertAlias('https://fcm.googleapis.com/fcm/send/A', 'https://fcm.googleapis.com/fcm/send/%41', 1); await assertAlias('https://fcm.googleapis.com/fcm/send/%7e', 'https://fcm.googleapis.com/fcm/send/~', 4); await assertAlias('https://fcm.googleapis.com/fcm/send/%2f', 'https://fcm.googleapis.com/fcm/send/%2F', 7); await assertAlias('https://fcm.googleapis.com/fcm/send/query?value=%41', 'https://fcm.googleapis.com/fcm/send/query?value=A', 10); const distinct = await preparePushSubscriptionSnapshot([row(20, 1, 'https://fcm.googleapis.com/fcm/send/reserved%2Fpath'), row(21, 2, 'https://fcm.googleapis.com/fcm/send/reserved/path'), row(22, 3, 'https://fcm.googleapis.com/fcm/send/query?value=%2F'), row(23, 4, 'https://fcm.googleapis.com/fcm/send/query?value=/'), row(24, 5, 'https://fcm.googleapis.com/fcm/send/order?a=1&b=2'), row(25, 6, 'https://fcm.googleapis.com/fcm/send/order?b=2&a=1')]); expect(distinct.invalidUserIds).toEqual([]); expect([...distinct.byUser.keys()]).toEqual([user(1), user(2), user(3), user(4), user(5), user(6)]); }); });
