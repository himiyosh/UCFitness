...
export async function preparePushSubscriptionSnapshot(rows: unknown[]): Promise<PreparedPushSubscriptions> {
    const byUser = new Map<string, StoredPushSubscriptionData[]>(); const userIds = new Set<string>(); const invalid = new Set<string>(); const capped = new Set<string>(); const counts = new Map<string, number>(); const rowIds = new Set<string>(); const endpointOwners = new Map<string, { count: number; userIds: Set<string> }>(); const identities: Array<{ row: Record<string, unknown>; id: string; userId: string }> = [];
    for (const row of rows) { if (!isRecord(row) || typeof row.id !== 'string' || !UUID_PATTERN.test(row.id) || typeof row.user_id !== 'string' || !UUID_PATTERN.test(row.user_id) || rowIds.has(row.id)) pushBoundaryFail('data'); rowIds.add(row.id); userIds.add(row.user_id); const count = (counts.get(row.user_id) ?? 0) + 1; counts.set(row.user_id, count); if (count > MAX_PUSH_SUBSCRIPTIONS) capped.add(row.user_id); if (typeof row.endpoint === 'string') { const owner = endpointOwners.get(row.endpoint) ?? { count: 0, userIds: new Set<string>() }; owner.count++; owner.userIds.add(row.user_id); endpointOwners.set(row.endpoint, owner); } identities.push({ row, id: row.id, userId: row.user_id }); }
    for (const { count, userIds: owners } of endpointOwners.values()) if (count > 1) for (const userId of owners) invalid.add(userId);
    const validated: Array<{ userId: string; subscription: StoredPushSubscriptionData }> = [];
    for (const { row, id, userId } of identities) { const agent = row.user_agent; const createdAt = row.created_at; const p256dh = typeof row.p256dh === 'string' ? row.p256dh : null; const auth = typeof row.auth === 'string' ? row.auth : null; const validKeys = p256dh !== null && auth !== null ? await isValidPushSubscriptionKeys(p256dh, auth) : false; if (!isAllowedPushEndpoint(row.endpoint) || !validKeys || p256dh === null || auth === null || (agent !== null && typeof agent !== 'string') || typeof createdAt !== 'string' || !Number.isFinite(Date.parse(createdAt))) { invalid.add(userId); continue; } validated.push({ userId, subscription: { id, endpoint: row.endpoint, p256dh, auth, user_agent: agent, created_at: createdAt } }); }
    for (const { userId, subscription } of validated) { if (capped.has(userId) || invalid.has(userId)) continue; const subscriptions = byUser.get(userId) ?? []; subscriptions.push(subscription); byUser.set(userId, subscriptions); }
