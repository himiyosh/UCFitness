...
import type { NotificationDeliveryClaimRpcArgs, NotificationDeliveryClaimRpcRow, NotificationDeliveryCompleteRpcArgs, NotificationDeliveryReleaseRpcArgs } from '@/types/database';
...
export const NOTIFICATION_DELIVERY_FAILURE_CODES = [
    'SOURCE_DATA_UNAVAILABLE', 'PAYLOAD_BUILD_FAILED', 'PUSH_DELIVERY_FAILED', 'PUSH_DELIVERY_INCOMPLETE',
] as const;
const MAX_CLAIM_USERS = 20;
const DATE_KEY_PATTERN = /^(?!0000)\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01])$/;
...
export interface ClaimNotificationDeliveriesOptions {
    notificationType: NotificationDeliveryType; occurrenceKey: string; userIds: readonly string[]; leaseOwner: string;
}
export interface CompleteNotificationDeliveryOptions {
    notificationType: NotificationDeliveryType; occurrenceKey: string; userId: string; leaseOwner: string; claimToken: string;
}
...
function isoWeekKey(dateString: string): string {
    ...
    if (!date) throw outboxError('occurrenceInput');
    date.setUTCDate(date.getUTCDate() + 4 - (date.getUTCDay() || 7));
    const isoYear = String(date.getUTCFullYear()).padStart(4, '0');
    const yearStart = parseDateKey(`${isoYear}-01-01`);
    if (!yearStart) throw outboxError('occurrenceInput');
...
export function buildStepReminderOccurrenceKey(date: Date): string {
    ...
    const key = getJSTDateString(date);
    if (!isCanonicalDateKey(key)) throw outboxError('occurrenceInput');
    return key;
}
...
function assertOccurrence(
    ...
    if (!isNotificationDeliveryType(notificationType)
        || !isValidNotificationOccurrenceKey(notificationType, occurrenceKey)) throw outboxError(errorKey);
}
...
function assertFence(options: CompleteNotificationDeliveryOptions, errorKey: ErrorKey): void {
    ...
    if (!isValidUUID(options.userId) || !isValidUUID(options.leaseOwner)
        || !isValidUUID(options.claimToken)) throw outboxError(errorKey);
}
