...
const secret = process.env.DASHBOARD_E2E_LOCAL_SECRET;
...
const token = secret ? await encode({
  token: {
    id: "11111111-1111-4111-8111-111111111111", sub: "fixture-provider",
    provider: "fitbit", provider_account_id: "fixture-provider",
    username: "fixture-runner", name: "Fixture Runner",
    email: "fixture@example.invalid", language: "ja",
  },
  secret, salt: "authjs.session-token", maxAge: 3600,
}) : null;
const mission = {
  id: "mission-1", mission_type: "WALK_100", title: "100 steps",
  description: "Walk 100 steps", reward_uc: 5, is_completed: true,
  completed_at: "2026-07-24T00:00:00.000Z",
};
const gear = {
  asin: "B000000001", title: "Lightweight walking shoes",
  image_url: "https://images.example.invalid/shoes.jpg",
  affiliate_link: "https://www.amazon.co.jp/dp/B000000001?tag=ucfitness-22",
  count: 2, users: [{ username: "walker-one", image: null, comment: "Easy to wear" }],
};
...
try {
  for (const viewport of [{ name: "mobile", width: 375, height: 812 }, { name: "desktop", width: 1280, height: 900 }]) {
    ...
    await page.addInitScript(() => sessionStorage.setItem("ucfitness:affiliate-experiment:f008-c3-v1", JSON.stringify({ schema: 1, positionVariant: "B", copyVariant: "B" })));
    ...
    assert.equal(await product.getAttribute("data-affiliate-position"), "B");
    assert.equal(await product.getByText(/Amazon.co.jpで商品を確認|Check this item on Amazon.co.jp/).count(), 1);
    ...
    assert.deepEqual(analyticsEvents.find((event) => event.event === "click"), {
      ...
      positionVariant: "B", copyVariant: "B", surface: "dashboard",
      targetType: "product", targetId: "B000000001",
    });
    assert.equal(imageFailures, 2);

    const close = page.getByRole("button", { name: /閉じる|Close/ });
    await close.waitFor();
    const controls = page.locator(".home-mission-module button,.trending-gear-module button,.trending-gear-module a[href],.login-bonus-toast button");
