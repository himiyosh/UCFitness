...
- GROUP challenge creation reauthorizes the supplied session user as a current OWNER or ADMIN inside a service-role-only database function, then creates the challenge and creator participation in one transaction. Until that unapplied migration exists in an environment, creation fails explicitly rather than falling back to split writes; large aggregation, all-member idempotent rewards, and UI remain Phase 2B work.
