...
- When a dashboard column combines a compact action dock with a data panel beside a taller peer panel, align the column and peer bottoms at the multi-column breakpoint. Keep five activity-or-discovery rows so the small remainder is distributed across meaningful rows instead of enlarging one real user row or leaving a trailing blank band.
