...
- 公開プロフィール対象のAchievement進捗・年間歩数、フォロー状態、公開リアクションでUUIDをDB照会前に共通検証
- プロフィール画像とバナーの保存拡張子を検証済みMIMEから決定し、Storageの`contentType`と一致
...
- グループ削除前のメンバー取得失敗は5xxで停止し、削除後は残存membershipを一括取得して重複を除いた影響ユーザーだけレガシーkeywordを同期。削除後の読取・個別更新失敗は部分障害として記録
