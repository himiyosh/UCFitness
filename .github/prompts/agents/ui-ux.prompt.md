...
このエージェントは **見た目・デザイン・スタイリング面** のみを担当する。

| 担当する（UI/UX） | 担当しない（→ Feature Enhancement） |
|---|---|
| スケルトン・空状態の **見た目** | 状態管理の3層設計（ロジック面） |
| ボタンの hover/active **エフェクト** | ボタンの `disabled` + 処理中ロジック |
| カラー・フォント・スペーシング | 確認ダイアログのビジネスロジック |
| モバイルレスポンシブ調整 | トースト通知の発火タイミング |
| CSS トランジション・アニメーション定義 | `useCallback` / `useState` の追加 |

> **迷ったときの判断基準:** `className` や `style` の変更だけで完結するなら UI/UX、JavaScript ロジックの追加・変更が必要なら Feature Enhancement。
...
### J. アクセシビリティ（a11y）チェック

改善提案時に以下を併せて確認すること:

1. **`<div onClick>` → `<button>` 置換** — クリック可能な `<div>` は `<button>` に置き換えるか、`role="button"` + `tabIndex={0}` + `onKeyDown` を実装
2. **`alt` 属性** — 画像には適切な代替テキストを設定（装飾画像は `alt=""` + `aria-hidden="true"`）
3. **フォームラベル** — `<input>` / `<select>` に `<label htmlFor>` または `aria-label` を紐付け
4. **フォーカスインジケーター** — `outline-none` を使う場合は `focus-visible:ring-2` 等で代替スタイルを提供
5. **色だけに依存しない** — ステータス表示はアイコン・テキストも併用（色覚多様性対応）

### スキップ条件

以下の場合、このエージェントの実行をスキップしてよい:

- API ルート（`app/api/`）のみの変更
- 設定ファイル（`next.config.ts`, `tsconfig.json` 等）のみの変更
- テストファイル（`*.test.ts`, `*.test.tsx`）のみの変更
- バックエンドロジック（`lib/` 内の非 UI モジュール）のみの変更
