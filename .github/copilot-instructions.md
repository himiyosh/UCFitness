...
- **デスクトップでも root スクロールを殺さない（0.9x スケーリング制約）**: `html { overflow: hidden }` の両方向適用は**全面禁止**（モバイル・デスクトップ問わず）。0.9xスケーリング使用時は `body` を固定高さ (`height`) のスクロールコンテナにし、`min-height` は使わないこと。`min-height` + `overflow-y: auto` の組み合わせではスクロールバーが出ない（body がコンテンツに合わせて伸びるため）。リファレンス: `app/globals.css` のデスクトップスケーリングセクション
