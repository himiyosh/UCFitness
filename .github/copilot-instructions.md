...
- **スクロール制御の原則**: ビューポート高さ (`100vh`, `100dvh`) や固定高さ (`h-[Npx]`) + `overflow-hidden` でコンテンツを固定範囲に閉じ込めるパターンは、ブラウザクロム・ツールバー・デバイスにより実際の表示領域が変動するため、見切れの原因になる。原則としてページ全体は自然スクロールに任せ、`overflow-hidden` + 固定高さはモーダルやドロップダウンなど「意図的に領域を限定する」コンポーネントのみに使用する。デスクトップ0.9xスケーリングは `globals.css` の `@media (min-width: 1024px)` で管理されており、そのセクション内の overflow 制御はスケーリングに必要な最低限であり、他のコンポーネントが参照すべき「一般ルール」ではない。
