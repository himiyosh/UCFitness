...
### LL-064: coverage分母と通常suiteの所要時間を別条件として検証しなかった

- **事象**: F026の60%回帰ゲートを追加した際、`coverage.include`を指定せず未importの本番`lib/`ファイルを分母から除外した。修正後はcoverage実行の18.10秒を正本の「Vitestで5秒以内」へ誤適用し、通常suiteが2.01〜2.75秒で完走する事実を分けて確認しなかった。
- **根本原因**: Vitestの既定coverage対象と、verification stepが指す通常テストの性能条件を独立した検証軸として定義せず、coverage付きCI suiteの所要時間を通常suiteの基準へ流用した。
- **対策**: `coverage.include`へ`lib/**/*.{ts,tsx}`を明示し、テスト、test-utils、型定義だけを除外する。all-filesの4指標はcoverage実行で、5秒条件はdefault parallelの通常`npm test`で別々に実測し、両方の結果を台帳とREADMEへ記録する。
- **教訓**: coverage gateは分母となる本番ファイル集合を明示して初めて回帰防止になる。verification stepの性能基準は、同じツールでもcoverage instrumentationやworker制約を加えた別コマンドへ置き換えず、記載された通常経路で測る。リファレンス: `vitest.config.ts`, `.github/ucfitness-features.json`, `README.md`
