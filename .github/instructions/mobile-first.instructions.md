...
- **モバイルファースト**: まずモバイル（`w-full`, `flex-col`）でレイアウトし、内容が収まる実コンテンツ幅に応じて`sm:` / `md:` / `xl:`で拡張
...
- ボタン・リンク・input・select・summary・カルーセルドットは最低 **44×44px** のタップ領域を確保する（小さなvisualは44px操作領域内へ配置）
...
| グリッド   | `grid-cols-1` | `md:grid-cols-2 xl:grid-cols-3` |

- `lg`でSidebarが出現する認証ページは、1024px時点の本文幅が約768pxまで縮む。Sidebar出現と3列以上への多列化を同時に行わず、複雑な構成は`xl`へ遅らせる
- 1023/1024pxと1279/1280pxで、viewport幅ではなく対象コンテナとカードの実幅を比較する
- 通常ページはdocumentの自然スクロールを使い、固定`max-height` + `overflow-y-auto`による二重縦スクロールを作らない
