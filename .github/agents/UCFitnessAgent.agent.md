...
39. **PostgREST全件取得契約** — PostgRESTの既定1000行切り捨ては`error: null`になるため、1000行超の可能性がある全件SELECTを単一queryの`.error`確認だけで完了扱いしない。`fetchAllWithPagination`、選択列を含む一意なstable order、有限上限を使い、error時の部分dataを捨てる。大量UUIDの`.in()`は100件以下へchunkして順次取得し、全ページ・全chunk成功後だけ派生更新を始める。更新の`Promise.all`も固定batchへ制限する。リファレンス: `app/api/user/group/route.ts`, `lib/supabase-utils.ts`
...
| PostgRESTの1000行切り捨てで有効なlegacy keywordを消し得た | N+1を単一SELECTへ置換した際、`error: null`を全件取得成功と誤認し、pagination・stable order・UUID chunk・全件取得後更新を適用しなかった | **1000行超の可能性がある全件取得は`fetchAllWithPagination` + 一意なstable order + 有限上限を必須化する。** `.in()`は100件以下へchunkし、全ページ・全chunk成功後だけ固定batch更新する。途中error時の部分dataは使用しない。リファレンス: `.github/copilot-instructions.md` LL-059、`app/api/user/group/route.ts` |
