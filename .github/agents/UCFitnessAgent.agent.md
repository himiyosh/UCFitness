...
| 2カラムグリッドの高さ不一致による空白（midnight テーマで顕在化） | ダッシュボードの 2 カラムグリッドで左カラム (StepCalendar のみ) が右カラム (DailyMissions + QuickActions) より短く、左カラム下に暗い背景の空白が発生。`items-start` は設定済みだったが、midnight テーマではカード外の暗背景とページ背景が同化し「カード内の空白」に見えた。Self-Critique の 6 軸批判でも検出されなかった | **2 カラムグリッドの左右カラムは高さを近づける**: 短いカラムにコンテンツ（DashboardChallenges 等）を追加して高さバランスを改善する。`items-start` だけでは不十分 — 特に midnight テーマでは背景の同化で空白が目立つ。**Self-Critique の「余白・密度」軸に「2 カラムグリッドの高さ不一致チェック」を追加**すること |
