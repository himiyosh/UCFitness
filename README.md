...
- **Timezone-safe date-only parseガード**: `app` / `components` / `contexts` / `hooks` / `lib` / `types`のproduction TS/TSXをTypeScript ASTで走査し、`new Date`と`Date.parse`へ渡す完全timestamp以外の静的文字列、`start_date` / `end_date`、offsetなしのtemplate/binary連結、単独の動的文字列、動的文字列へoffsetだけを後置した構築をdefault-denyで拒否する。既存`Date`、epoch number、完全timestamp、timestamp field、1つの日付slotへ静的時刻と明示`Z` / `+09:00` / `+0900`を組み合わせた構築、共有JST helperは許可し、コメント、docs、test/spec、fixtureは対象外とする。date-onlyの順序比較は検証済み`YYYY-MM-DD`文字列、日付演算・表示は明示UTC/JST、Challenge/Group Eventの操作可否は`getChallengeScheduleMetrics`を正本にする
