...
- **整数queryの入力契約**: `/api/user/analytics`の`months`は省略時3、指定時1〜12、`/api/user/step-calendar`の`year`は省略時現在年、指定時2000〜2100だけを受理する。入力は空白なしの10進整数全文（先頭の`+`/`-`は構文上許可）かつsafe integerに限定し、小数・指数・16進・部分一致・空文字・範囲外をclampや既定化せず400で拒否する
