...
- **通知品質契約**: `users.language`から生成したja/en文言をRFC 8291暗号化payloadで端末へ届ける。バッジは個人・全体・グループをユーザー単位1通へ統合し、同一UA/legacy購読は最新1件だけを送信対象にする。404/410件数は`expired`を維持し、cleanupは送信時に観測した`id`、`user_id`、`endpoint`、`p256dh`、`auth`、`user_agent`、`created_at`を`delete_push_subscription_if_unchanged`へ渡して主キーrow lock下で全項目が一致した場合だけ`pruned`、不一致・既削除は`preserved`、RPC障害は固定`cleanupFailed`として他端末の送信を継続する。Push `Topic`とNotification `tag`で同種通知を置換し、通知ベルの集約単位と未読数も一致させる
- **Push cleanup migration運用 (LL-080 / MERGE BLOCKED)**: `migrations/20260725_delete_push_subscription_if_unchanged.sql`は`20260720_harden_push_subscriptions_rls.sql`の後、PR #300/#301より先に適用する。現PRのtext testsは補助であり、別stacked PR「CAS runtime PostgreSQL verification」が既存`pg` dependencyとGitHub Actions PostgreSQL serviceでdefault/ACL/arbiter negative fixtures、exact delete/stale false、update-first/delete-first二接続競合を実行して先にmergeされるまで本foundationをmergeしない。rollbackは依存コードを先に停止・撤去してから7引数RPCのEXECUTE revoke後にDROP FUNCTIONする。本PRはmigrationを本番適用しない
...
PUSH_CAS_POSTGRES_TEST_ONLY=1 PUSH_CAS_POSTGRES_URL=postgresql://postgres:postgres@127.0.0.1:5432/postgres npm run test:postgres:push-cas
```
