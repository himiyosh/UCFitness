...
- **F006 migration運用**: Phase 3A rollbackは`REVOKE EXECUTE ON FUNCTION public.get_group_challenge_progress(uuid, uuid) FROM service_role;`後に`DROP FUNCTION IF EXISTS public.get_group_challenge_progress(uuid, uuid);`を実行する。Phase 3B1は途中の歩数減少・欠測による先払いを避けるため、終了日の翌日以降に`settle_group_challenge`で単一challengeを精算する。Phase 3B2の`/api/cron/group-challenge-settlement`はJST終了済み・未精算GROUP候補を安定順で20件ずつ取得し、RPCを逐次実行する。RPCはchallengeを排他lockし、現member集合・期間内正歩数・全member入金・participant更新・`settled_at`を単一transactionで確定する。rollbackはCron呼び出し停止後、(1) 3B1 RPCのREVOKE/DROP、(2) `credit_balance`をPhase 3A定義へ復元、(3)取引type constraint復元、(4)settlement constraint/列削除、(5) Phase 3A RPC、(6) Phase 2A RPCの順とする。Phase 2A/3A/3B1 migrationとCron scheduleはproduction未適用。達成通知のPush実装は別blockerであり、このCronには含めない
