...
- **Date-only JST終了境界ガード**: productionのTS/TSXで`new Date()`へoffsetなしの`T23:59:59`を連結する実式をTypeScript ASTで拒否し、明示的な`Z`/UTC offsetは許可する。コメント、docs、test/spec、fixtureは検査対象外にし、Challenge/Group Eventの表示・分類・操作可否は`getChallengeScheduleMetrics`へ委譲する
