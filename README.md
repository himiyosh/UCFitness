...
| **プッシュ通知** | 言語対応Web Push、バッジ集約、歩数リマインダー、JST前週サマリー、端末重複制御。共有server正本`lib/api/web-push.ts`は`loadPushSubscriptionSnapshot`・`preparePushSubscriptionSnapshot`・`isValidPushSubscriptionKeys`・`sendWebPushNotifications`でexact count付き単一statement最大900件、raw user20、WebCrypto鍵検証、active20/15秒timeoutを提供し、超過・count欠落・件数不一致は部分処理せずfail closedとする。第一identity passではcredentialなしHTTPS endpointのfragmentを除くRFC3986正規化済み`url.href`でglobal所有権を検査し、unreserved escape aliasは同一視する一方、reserved/UTF-8 byte列・query orderのsemantic diffを保持する。same-user/cross-userの重複endpointは関係user全員を隔離し、global重複row IDは全体data errorとして拒否する。subscribe境界は固定non-PII AppErrorだけを一回reportし、raw backend errorやuser/endpoint/key値を渡さない。CAS migration/runtimeは#305/#306、outbox migration/runtime/serviceは#308/#309/#311、recipient-generation migration/runtimeは#310/#312でmainへ統合済み。残るblockerはproduction CAS/outbox RPCのapply・read可用性、CAS app wiring Draft #307、recipient-generation server Draft #314、client/SW Draft #315とsender protocol readinessであり、PR #300/#301/#313/#316はこれらが揃うまでDraftを維持する。TTL 300のdelivery lease、Topic/tag、単純ownership transferは遅延配信中privacyを解決しない |
| **i18n** | 日本語・英語の 2 言語対応 |
...
- **通知品質契約**: `users.language`から生成したja/en文言をRFC 8291暗号化payloadで端末へ届ける。歩数リマインダーはPR #300の共有`loadPushSubscriptionSnapshot` / `preparePushSubscriptionSnapshot` / `sendWebPushNotifications`を正本とし、single-statement exact count最大900件、validation前raw20端末、global endpoint ownership quarantine、WebCrypto P-256/auth16、active20端末、15秒abortを継承する。Layer 3CはJST日`YYYY-MM-DD`をoccurrenceとしてsubscription candidateをUUID安定順・最大20件で先にclaimし、claim済みユーザーだけのprofile/goal/stepsを読む。DB成功時の歩数行欠落だけを0歩とし、invalid/missing profile/goal、invalid/foreign/unsafe steps、batch query failureはexact owner/tokenでreleaseして最大5 attemptへ収束させる。70%以上は実送信なしでもoccurrence resolvedとしてterminal completeし`suppressed`へ計上する安全な例外、1端末以上成功は`completedDelivery`、0端末成功・payload/Push失敗はallowlist codeでreleaseする。`failed`は端末、`failedUsers`はユーザー、`outboxFailures`はRPC/fence失敗として分離し、completed/active/max-attemptのclaim欠落は正常skipとする。全suppressed時は`success:true`でも「送信完了」と呼ばず「通知対象者なし（目標圏内）」を返す。成功Pushからcompleteまでのcrash windowを持つat-least-once制御でありexactly-onceは主張しない。PR #301/#316はproduction CAS/outbox RPCのapply・read可用性、Draft #307/#314/#315とrecipient protocol readinessが揃うまでmerge禁止とする
...
claimは1〜20件の入力userをUUID安定順で`FOR UPDATE`し、台帳を同一transactionで作成・lockして、pendingまたは期限切れclaimedだけへ新tokenを発行する。active claim・completed・failedはskipし、返却はuser IDとclaim tokenだけとする。Layer 3はpersonalized data取得・Push送信より前にclaimし、実配信成功は`completedDelivery`、有効データで通知不要なら実送信なしのterminal `suppressed`としてcompleteする。releaseはsource/profile/steps/payload/Push失敗だけを所有中の未期限切れtokenへ記録し、最大5 attemptで収束させる。

static/runtime testはmigration SHA-256、catalog/default/FK/index/owner/RLS/ACL、lock順、stale token、5 attempt、JST日/ISO週境界、90日保持、二接続競合、rollbackを固定済みである。#313/#316はcanonical wrapperだけを各routeから呼び、production outbox RPCのapply・read可用性を明示確認するまでDraftを維持する。rollbackはLayer 3を停止してactive leaseを待ってから`release→complete→claim→index→table`の順にREVOKE/DROPし、過去migrationは編集しない。

Layer 3Aの`lib/services/notification-delivery-outbox.ts`はserver-only typed wrapperとしてmainへ統合済みであり、Step Reminder routeがproduction importerとしてclaim/complete/releaseを各1 callsiteだけ使用する。wrapperはexact RPC引数、最大20 UUID安定dedup、JST日/ISO週key、unknown返却shape、owner/token fencing、固定release failure code、内部`reportError`なし、raw Error/UUID非露出を担う。production outbox RPCは未適用のため、Draft配線を実配信可用性の根拠にしない。
...
| Cron / integration / debug | `app/api/cron/step-reminder/route.ts` (1), `lib/services/weekly-summary.ts` (1), `app/api/external/ranking/route.ts` (1), `app/api/notify-teams/route.ts` (1), `app/api/debug/db-check/route.ts` (1) | cron secret / API key / sessionを各route/serviceで検証 |
