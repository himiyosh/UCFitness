...
Layer 3A2は`lib/push-endpoint.ts`をpureなcanonical ownership key正本とし、credentials拒否、fragment除去、host/default portとRFC 3986 percent encodingを正規化する。helperはcanonical keyを2048文字以下に制限し、既存`isAllowedPushEndpoint`は保存・送信用raw endpointの2048文字上限も別に維持する。`PushPayload`はauthority fieldを`never`で禁じるgeneric variantとgeneration/version/protocolを全必須にするprivate symbol branded authority variantへ分離し、`withPushRecipientAuthority`だけがauthority payloadを構築する。single/batch senderは暗号化・network前にbrandと3 fieldをall-or-noneで検証し、不正形は固定非PII `AppError`で拒否する一方、既存generic callerは変更しない。本Layerではsubscribe route、ownership wrapper、sender callsiteを配線せず、PR #314/#315と旧worker排出が揃うまでpersonalized送信はMERGE BLOCKED、ownership/protocol migrationはproduction未適用のままとする。
