...
- **通知Feed追加境界契約**: `ActivityFeed`と`NotificationBell`は追加ページをReact state updater外で集約し、latest feed ref・request generation・AbortController・mount guardで後着応答とunmount後更新を拒否する。不正timestampは既存ja/enエラー表示へ分離してerror boundaryへ投げず、`GET /api/user/feed`のサーバー事前集約も不正値をページング応答へ流出させない防衛線として維持する
...
PR #314はLayer 3A1 wrapperをsubscribe routeへ接続し、Client/SWがackしたnumeric integer `recipientProtocolVersion: 1`だけを受けてgeneration/version/protocolを返す。canonical key生成とpayload authorityは`lib/api/web-push.ts`へ残し、weekly/step senderは未配線のままとする。Draft merge blockerはproduction ownership/protocol migrationの適用とread RPC可用性、PR #315のactive Service Worker ack/state、weekly/step sender generation wiring、旧Service Worker rollout、legacy client互換性であり、このPRではproduction DB操作・実Push・Cron callsiteを行わない。
...
- WalkingRoutesの既存flakeはstep上限不足ではなく削除ダイアログのfocus競合だった(base `7992e07b`)。失敗は常に削除後のroute数待機`components/WalkingRoutes.test.ts:413`で、`useDialogFocus`が初期フォーカスを`requestAnimationFrame`でCancelへ当てるため、dialog表示直後にDeleteへ`press('Enter')`するとPlaywrightのfocusと押下の間にそのフレームが割り込み、Enterの発火先がCancelへ移る。dialogは削除せずに閉じ、route数が減らないため待機はtimeoutまでハングする。step上限を5秒から15秒へ上げても再現した(15000ms timeoutを実測)ことがduration非依存の競合である証拠で、上限拡大は対処にならない。押下前にCancelへの初期フォーカス確定を待つ待機を追加して修正し、step上限は5秒のまま維持した。他のdialog操作は`click()`かDOM直接clickでfocus順序に依存しないため変更していない。同一host・同一負荷(busy loop 10本、load average 16〜20)のfocused cohortは修正前3/12 FAIL(全て`:413`の5000ms timeout)、修正後0/24 FAILで、静穏hostでは修正前12/12 PASSと差が出ない。full suiteは120 files・1588 tests・skipped 0をPASSし、single-worker coverageは32.38秒でStatements 71.18% / Branches 68.46% / Functions 81.19% / Lines 72.45%の60% gateを維持した。5秒条件は未達のためF026は`in-progress`を維持する
