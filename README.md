...
- **F006 migration運用**: Phase 3A rollbackは`REVOKE EXECUTE ON FUNCTION public.get_group_challenge_progress(uuid, uuid) FROM service_role;`後に`DROP FUNCTION IF EXISTS public.get_group_challenge_progress(uuid, uuid);`を実行する。全memberへの冪等報酬は残blockerであり、Phase 3Aには含めない
