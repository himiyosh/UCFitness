...
Layer 3Aは`lib/api/web-push.ts`の`getPushEndpointOwnershipKey`をsubscribeと将来のCronが共有するcanonical SSoTとし、`lib/services/push-subscription-ownership.ts`のserver-only境界からexact save/read/release RPCだけを呼ぶ。POSTはdirect upsertを廃止し、同一UA/legacyの古い行だけを既存CAS RPCで整理して、既存`success`/`pruned`に`recipientGeneration`/`recipientVersion`を追加する。DELETEはsession user＋raw endpointからcurrent rowを限定取得してread RPCのgeneration/versionをrelease RPCへfenceし、authority未作成のlegacy行だけは全列一致CASで解除する。payload helperはgenerationをRFC 8291 JSONへ含められるが既存Cronは未配線であり、production migration適用・read RPC可用性確認・Layer 3BのSW/client generation保存/照合/旧payload dropが揃うまでDraftかつMERGE BLOCKEDを維持する。

Layer 3BはService WorkerのCache Storageへ`generation/version/protocolVersion/tombstone`だけを保存し、Web Locksのorigin-wide exclusive lockでsubscribe/rebind/release/sign-in/logoutを直列化する。logoutはlock内でclear→`signOut({redirect:false})`→clear→navigate、releaseはDELETE後に最新transition tokenを再検証してからunsubscribeする。SWはprotocol v2のMessageChannel ack、clearごとの非永続token、clear/set/verify/push直列queueを使い、personalized payloadをgeneration/version/protocol完全一致時だけ表示する。認証状態・user IDはclient memoryだけで監視し、既存購読を初回/ユーザー変更時に必ずclear-before-POST rebindする。protocol readinessのDB保存/read/filter用additive migration、weekly/step sender配線、production RPC適用/read可用性、実Chromium push harnessのstackが揃うまでDraftかつMERGE BLOCKEDを維持する。
