...
Layer 3BはService WorkerのCache Storageをorigin-localなrecipient authorityとし、generationと単調version、clear後のversion tombstoneを再起動・複数タブ間で共有する。clientはMessageChannelのackと固定timeoutを待ち、subscribe/transfer/DELETE/signOut前にdurable clearを完了する。SWは全clear/set/pushを直列化し、clearごとの非永続transition tokenを一致させたPOST応答だけをsetするため、未確定の将来versionを持つ旧応答もSW再起動時はfail closedで拒否する。weekly summary、step reminder、badge、group challenge reward、test通知はgeneration/version exact一致時だけ表示し、missing・invalid・旧世代を非PII診断だけでdropする。generic通知は維持するが、weekly/step sender配線、production RPC適用とread可用性確認が未完了のためDraftかつMERGE BLOCKEDを維持する。
