...
- **バナー画像編集のgeometry契約**: 2.5:1のcrop高さ・bleed・offset clamp・ズーム中心は現在のpreview幅から同じpure helperで算出する。ResizeObserverでは幅比でoffsetを補正して元画像上の選択中心を保持し、画像比率由来のdynamic minimum scaleとeffective maxでcrop枠の空白を防ぐ。編集面は`touch-action: none`と`passive: false`のnative wheel listenerを局所適用し、pointer・keyboard・wheel・slider・保存は最新stateと操作時DOM幅を使う。upload先、MIME、5 MB上限、JPEG圧縮契約は変更しない
