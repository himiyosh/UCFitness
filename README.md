...
| **プッシュ通知** | 言語設定対応のWeb Push、バッジ横断集約、歩数リマインダー、JST前週とUTC半開区間を一致させたウィークリーサマリー、端末重複制御。週次通知は全購読をUUID keysetで900件ずつ取得し、20ユーザー単位でDB/shape障害を分離、部分失敗は成功済み件数を保持した503でCron再試行へ伝播 |
| **i18n** | 日本語・英語の 2 言語対応 |
