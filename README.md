...
Layer 3A2は`lib/push-endpoint.ts`をpureなcanonical ownership key正本とし、credentials拒否、fragment除去、host/default portとRFC 3986 percent encodingを正規化する。helperはcanonical keyを2048文字以下に制限し、既存`isAllowedPushEndpoint`は保存・送信用raw endpointの2048文字上限も別に維持する。generic payload shapeはauthority keysを宣言せず、single/batchのconst-generic overloadがactual argumentの`keyof T`を検査するため、`exactOptionalPropertyTypes: false`でも明示`undefined`を含むauthority keyはcompile rejectされ、private symbol branded helper outputだけがauthority overloadを通る。senderは最初のawait前にpayload・subscription・stored rowをaccessor/inherited/unknownなしのsnapshotへ一度コピーして即JSON/UTF-8化する。Phase Aで全deviceのVAPID JWT、P-256 import、暗号化、immutable RequestInitを完了し、1件でも失敗すればfetch 0の固定`AppError`、Phase Bだけがprepared requestをfetchする。404/410 cleanupはprepared requestが保持したinitial id/endpoint/key/UA/created_atをCAS RPCへ渡し、original mutationを再読しない。既存generic callerはreturn shapeを維持し、本Layerではsubscribe route、ownership wrapper、sender callsiteを配線しない。PR #314/#315と旧worker排出が揃うまでpersonalized送信はMERGE BLOCKED、ownership/protocol/CAS migrationはproduction未適用のままとする。
