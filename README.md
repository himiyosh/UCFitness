...
- **GROUP Challenge認可契約**: public閲覧と参加を分離し、join/progress/leaveは現group member、PUTはcreatorかつ現OWNER/ADMINだけを許可する。GROUP作成UIはServer ComponentでOWNER/ADMINの管理グループだけを取得して必須選択するが、認可はservice-role専用`create_group_challenge` RPCでDB再検証する。Phase 2A migration未適用時は明示的な5xxとし、1000件超集計RPC・全member冪等報酬を残blockerとする
