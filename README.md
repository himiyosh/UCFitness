...
| 経路 | 現在の偽装 | 正しい障害境界 |
|---|---|---|
| `app/api/amazon/personalized/route.ts` | 修正前は残高・歩数のerror / null / 不正値をBEGINNER・0歩へ変換 | 残高行なしだけを新規ユーザー0として許可し、DB障害・不正shape・unsafe数値・予期しないrejectはPIIを含まない固定503で判定停止。空歩数と記録済み0歩は平均0として維持 |
