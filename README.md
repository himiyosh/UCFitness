...
- **F030招待リンク基盤**: `POST /api/group/invite`の`create`/`join`はEdge Runtimeで動作し、発行はOWNER/ADMINだけをDB RPC内で認可する。256-bit生tokenは成功時に一度だけ返し、DBにはSHA-256 hashと7日後の失効時刻だけを保存する。期限切れhashは410判定のため保持し、1グループ20件の有効招待上限で発行頻度を抑える。参加はグループ・ユーザー・招待をロックする単一RPCでmembershipと`group_keyword`を原子的に同期する。migrationは未適用のため、UI接続前に`migrations/20260719_add_group_invite_links.sql`を承認済み環境へ適用する
