...
| `lib/services/badge-awards.ts` | Phase Aでpersonalのactive users / users / batch totals / 30日履歴だけを固定AppErrorへ分離済み。生DB errorは内部logにも渡さずCron callerへ固定AppErrorだけを伝播。global/group ranking・award insertは未修正 | 次Phaseでglobal/group/award insertを個別に分離し、0歩・参加者なしと区別 |
