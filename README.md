...
Layer 1はendpoint本文を保持しないSHA-256 digest主キーの`push_subscription_ownership`を作り、owner、ランダム`recipient_generation`、version、時刻をDB正本にする。legacy backfillは同一endpointの全行が1 userへ一致する場合だけownerを設定し、cross-user重複は任意winnerを選ばずownerなしで隔離して件数だけを報告する。save/transfer RPCはendpoint advisory lock後に関係userをUUID順でlockし、raw 20件上限を原子的に検証してcross-user旧行を削除する。同一owner retryはgenerationを維持してversionだけを進め、未所有claim・所有者移転・generation＋version一致releaseはgenerationを回転する。既存CASはsubscription行だけをlockするため循環待ちを作らず、exact row-version比較により新winnerを削除しない。Push network待機中にDB transactionは保持しない。

Layer 3はpersonalized payloadへ`recipient_generation`を含め、Service Workerが端末へ保存したcurrent generationと一致する場合だけ表示する。logoutではgenerationをclearし、account switchまたは再subscribeでは更新し、遅延した旧generationとgenerationなしpayloadをdropする。同一generationのsave応答は最大versionだけを採用し、releaseへgenerationとversionを返して古い解除を拒否する。適用順は既存CAS→Layer 1→Layer 2のruntime PostgreSQL検証→Layer 3のgeneration binding deploy→personalized送信停止中の旧worker排出・新Service Worker有効化確認→cutover migration→送信再開であり、Layer 1時点は現行subscribe APIを壊さないため既存grantを維持する。cutover migrationは`push_subscriptions`を`ACCESS EXCLUSIVE`でlockし、backfill後のdirect writeを全digestから再同期してowner差分・曖昧行のgenerationを回転し、欠落・driftなしを確認してdirect write権限を同一transactionで撤去する。Layer 2はcross-user同時claim、同一userの19→20/20→reject、逆順transfer、user削除競合、generation回転、stale release、legacy重複、CAS競合、rollbackを実PostgreSQLで証明する。

ロールバックはLayer 3送信を停止して旧workerを排出後、`release_push_subscription_with_generation`、`save_push_subscription_with_generation`、`push_subscription_ownership_owner_idx`、`push_subscription_ownership`の順にREVOKE/DROPする。Layer 3適用後は先に旧direct write grantを必要最小限だけ復元し、既存`push_subscriptions`とCAS migrationは保持する。過去migrationを編集せず、generation付きqueued payloadが残る間はService Workerの比較処理を先に撤去しない。
