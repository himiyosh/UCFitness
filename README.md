...
- **整数入力の検証契約**: `/api/user/analytics`の`months`は省略時3、指定時1〜12、`/api/user/step-calendar`の`year`は省略時JST現在年、指定時2000〜2100だけを受理する。入力は空白なしの10進整数全文（先頭の`+`/`-`は構文上許可）かつsafe integerに限定し、小数・指数・16進・部分一致・空文字・範囲外をclampや既定化せず400で拒否する。Walking Routes POSTの`duration_minutes`は省略/`null`または非負safe integer、`distance_km`は省略/`null`または非負finite numberだけを受理し、型・小数（時間）・非finite値を`null`へ偽装せずDB照会前に400とする。Clientフォームも送信前に同じ整数正本で時間を検証し、不正値は修正方法を示す可視alertと`aria-invalid` / `aria-describedby`で入力へ関連付ける。native `badInput`回帰はrunner既設Google Chromeをskip/fallbackなしで使い、CI cold実測に基づく対象test 30秒と各操作5秒を分離してglobal timeoutを緩和しない
